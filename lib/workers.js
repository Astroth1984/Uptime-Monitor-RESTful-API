...
/*
 * workers is the start argument name on console
 * for logging & debugging puposes
 */
...
workers.gatherAllChecks = function() {
    ...
    _data.list('checks', function(err, checks) {
        if (!err && checks && checks.length > 0) {
            ...
            debug('Error: Could not find any checks to process', err);
        }
    });
            ...
            logs.forEach(function(logName) {
                ...
                _logs.compress(logId, newFileId, function(err) {
                    if (!err) {
                        // Truncate the Log
                        /*
                         * Taking content of .log file and moving it
                         * to compressed file
                         * then delete .log
                         */
