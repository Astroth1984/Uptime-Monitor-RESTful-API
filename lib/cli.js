...
// Dependencies
/*
 *  readline: built in node package to read a line
 */
var readline = require('readline');
var util = require('util');
var debug = util.debuglog('cli');
var events = require('events');
// Our own extended events class
...
cli.responders.help = function() {
    ...
    var commands = {
        ...
        'man': 'Show this help page - CLI manual',
        ...
        'List logs': 'Show a list of all the log files available to be read (compressed and uncompressed) in the system',
        'More log info --{logFileName}': 'Show details of a specified log file',
    };

    // Show a header for the help page that is as wide as the screen
    cli.horizontalLine();
    cli.centered('ADMIN CLI MANUAL');
    cli.horizontalLine();
    cli.verticalSpace(2);
...
cli.responders.listLogs = function() {
    // Get the list of logs directly from the shell : ls command
    ...
    ls.stdout.on('data', function(dataObj) {
        ...
        logFileNames.forEach(function(logFileName) {
            if (typeof(logFileName) == 'string' && logFileName.length > 0 && logFileName.indexOf('-') > -1) {
                console.log('\x1b[35m%s\x1b[0m', logFileName.trim().split('.')[0]);

            } else if (typeof(logFileName) == 'string' && logFileName.length > 0 && logFileName.indexOf('-') == -1) {
                console.log('\x1b[45m%s\x1b[0m', logFileName.trim().split('.')[0]);
...
cli.processInput = function(str) {
    ...
    if (str) {
        ...
        if (!matchFound) {
            console.log('\x1b[31m%s\x1b[0m', "Error: The command line argument is not valid. Verify the switch you are using --No Matched Command Line Found. Use -- man/help -- command for more details. ");
        }
...
cli.init = function() {
    ...
    console.log('\x1b[34m%s\x1b[0m', 'Admin CLI is running');
